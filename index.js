...
var onSteamLogOn = function onSteamLogOn(){
		...
		Dota2.on("ready", function() {
			...
			var pages = [];
			var p = 0;
			function getSourceGames(start_at){
				Dota2.findSourceTVGames({start : start_at}, function(data) {    // May 2015
					console.log('Successfully received SourceTVGames');
					if (data) {
						pages[p] = data;
						p++;
						getSourceGames(start_at+6)
					}else{
						MongoClient.connect("mongodb://localhost:27017/dota2", function(err, db) {
							Collections = {};
							Collections.Games = db.collection("games");
							Collections.Games.insert({pages: pages}, {w:1}, function(err) {});
						});
					}
